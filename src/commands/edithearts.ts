    ...
    run: async opts => {
        ...
        switch (option) {
            ...
            case 'add':
                for (const heart of value) {
                    ...
                    if (!stmt || !stmt.changes) {
                        ...
                        if (!succeeded.includes(heart))
                            succeeded.push(heart)
                    }
                }
                if (succeeded.length)
                    await opts.chatClient.say(opts.channel, `Added hearts (${succeeded.join(' ')}) to channel (${channel}).`);
                if (failed.length)
                    await opts.chatClient.say(opts.channel, `Failed to add hearts (${failed.join(' ')}) to channel (${channel}).`);
                break;
            ...
            case 'remove':
                ...
                if (succeeded.length)
                    await opts.chatClient.say(opts.channel, `Removed hearts (${succeeded.join(' ')}) from channel (${channel}).`);
                if (failed.length)
                    await opts.chatClient.say(opts.channel, `Failed to remove hearts (${failed.join(' ')}) from channel (${channel}).`);
                break;
            ...
            case 'toggle':
                ...
                if (!stmt || !stmt.changes)
                    ...
                    await opts.chatClient.say(opts.channel, `Toggled all hearts from channel (${channel}) to ${value[0]}.`)
                return;
        }
        return;
    }
