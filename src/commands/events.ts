...
export const command = {
    ...
    requiresInput: true,
    minArgs: 2,
    min_args_error_message: "Usage: `!events <query> [...value?]`. An example 'query' could be: action=toggle&event=raid",
    run: async opts => {
        ...
        switch (action.toLowerCase()) {
            case 'toggle': {
                if (!['true', 'false'].includes(value[0])) {
                    await opts.chatClient.say(opts.channel, "The value given for 'action:toggle' is not valid. This must be a Boolean!");
                    return;
                }

                const stmt = await toggleChannelEvent(channel, eventName, value[0] === 'true').catch(e => {
                    logger.setPrefix(LogPrefixes.DATABASE).error(e);
                    return;
                });

                let response = (!stmt || !stmt.changes)
            ...
            case 'edit': {
                ...
                if (!option || !['trigger', 'message'].includes(option)) {
                    await opts.chatClient.say(opts.channel, "You must declare which event option, you'd like to manage.");
                    return;
                }

                const input = (option === 'trigger') ? value[0] : value.join(' ');
                const cb = (option === 'trigger')
                    ? updateChannelRaidEventTrigger
                    : updateChannelRaidEventMessage;
